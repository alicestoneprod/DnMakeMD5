...
BOOL CDnMakeMD5Dlg::OnInitDialog()
{
	...
	m_szText = _T("Press the [Open] button to load the file for generating MD5, or drag it here.\nPress the [Save] button to create a file named after the loaded file with the .md5 extension.\nYou can also use the command line with DnMakeMD5.exe [output_filename].");
...
void CDnMakeMD5Dlg::OnBnClickedButtonOpen()
{
	...
	CFileDialog Dlg(TRUE, _T("*.*"), NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, _T("Files (*.*)|*.*||"), NULL);
...
void CDnMakeMD5Dlg::OnBnClickedButtonSave()
{
	...
	{
		AfxMessageBox(_T("Please load the file."));
		return;
	}
...
CDnMakeMD5Dlg::DropFile(HDROP hDrop)
{
	...
	if ( iCount != 1 )
	{
		AfxMessageBox(_T("Please drag only one file."));
		return;
	}
...
CDnMakeMD5Dlg::LoadFile(CString& str)
{
	...
	{
		m_szText = str;
		m_szText.Format(_T("%s\n\n[%s] \nSaved to clipboard."), str, m_szChecksum);
...
CDnMakeMD5Dlg::SaveFile(CString str)
{
	...
	{
		AfxMessageBox(_T("MD5 has not been generated."));
		return false;
	}
	...
	{
		m_szText = str;
		m_szText.Format(_T("%s has been saved.\n\n[%s] \n"), str, m_szChecksum);
		UpdateData(FALSE);
	}
